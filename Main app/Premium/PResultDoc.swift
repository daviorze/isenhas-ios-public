...
class PResultDoc:UIViewController {
    ...
    func updateItem(){
        ...
        if(selected_pass["old"] != nil){
            var myOld = selected_pass["old"] as? String ?? ""
            if(extreme){
                myOld = SwiftHelper.extremeEncrypt(myOld, SwiftHelper.getRecoveryKey()) 
            }
            postRecrod["old"] = myOld
        }
        if(isFavorite){
